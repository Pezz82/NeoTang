...
# Add Yosys to PATH
$env:Path += ";C:\Users\johnp\Downloads\yosys-build\bin"
...
Check-DirExists $RTL_DIR "RTL directory not found: $RTL_DIR"
...
$SOURCE_FILES = @(
    ...
    # RTL subdirectories
    (Get-ChildItem -Path "$RTL_DIR\cd" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\cells" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\cpu" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\io" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\jt12" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\jt49" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\mem" -Recurse -Include "*.v","*.sv","*.vh").FullName
    (Get-ChildItem -Path "$RTL_DIR\video" -Recurse -Include "*.v","*.sv","*.vh").FullName
    ...
    "$RTL_DIR\ip_stubs\altsyncram.v"
    "$RTL_DIR\ip_stubs\altera_pll.v"
    "$RTL_DIR\ip_stubs\altpll.v"
)
...
foreach ($file in $SOURCE_FILES) {
    $yosysScript += "read_verilog -sv -defer `"$file`"`n"
}

$yosysScript += @"
hierarchy -top neogeo_top
proc
flatten
opt
synth_gowin -top neogeo_top -json neotang.json
"@
